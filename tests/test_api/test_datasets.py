...
class TestColumnValues:
    async def test_returns_unique_sorted_values(self, client, uploaded_path):
        resp = await client.post(
            "/api/datasets/column-values",
            json={"dataset_path": uploaded_path, "column": "Region"},
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    async def test_group_column_has_five_values(self, client, uploaded_path):
        resp = await client.post(
            "/api/datasets/column-values",
            json={"dataset_path": uploaded_path, "column": "Group"},
        )
        values = resp.json()["values"]
        assert set(values) == {"1", "2", "3", "4", "5"}

    async def test_numeric_column_values_are_strings(self, client, uploaded_path):
        """API always returns values as strings, even for numeric columns."""
        resp = await client.post(
            "/api/datasets/column-values",
            json={"dataset_path": uploaded_path, "column": "ClaimNb"},
        )
        values = resp.json()["values"]
