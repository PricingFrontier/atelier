      ...
      <div className="grid grid-cols-4 gap-px bg-white/[0.03]">
        {entries.map(([key, val]) => (
          <div key={key} className="bg-background px-4 py-3">
            <p className="text-[0.6rem] uppercase tracking-wider text-muted-foreground/40">
              {key.replace(/_/g, " ")}
            </p>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {vif.map((v: any, i: number) => (
              ...
              >
                <td className="px-4 py-2 font-mono text-[0.75rem] text-foreground/70">{v.feature}</td>
                <td className={cn(
                ...
                <td className={cn(
                  ...
                  v.severity === "high" ? "text-red-400" : v.severity === "moderate" ? "text-amber-400" : "text-muted-foreground/40"
                )}>
                  {v.severity ?? "—"}
                </td>
                <td className="px-4 py-2 text-[0.7rem] text-muted-foreground/50">
                  {v.collinear_with ?? "—"}
                </td>
              </tr>
            ))}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {pairs.map((p, i) => (
              ...
              >
                <td className="px-4 py-1.5 font-mono text-[0.7rem] text-foreground/70">{p.a}</td>
                <td className="px-4 py-1.5 font-mono text-[0.7rem] text-foreground/70">{p.b}</td>
                <td className={cn(
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {factors.map((f: any, i: number) => (
              ...
              >
                <td className="px-4 py-2 font-mono text-[0.75rem] text-foreground/80">{f.name}</td>
                ...
                <td className="px-4 py-2 text-right font-mono text-[0.75rem] text-muted-foreground/60">
                  {f.type === "categorical"
